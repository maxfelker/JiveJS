...
d.textselect(!0),!1===b.click&&b.dragging&&a.data(b.mousedown,"suppress.click",(new Date).getTime()+5),b.dragging=d.touched=!1}},hijack:function(e,b,c,g,i){if(c){var k=e.originalEvent,q=e.type,n=b.indexOf("drop")?"drag":"drop",l,p=g||0,h,m,g=!isNaN(g)?g:c.interactions.length;e.type=b;e.originalEvent=null;c.results=[];do if((h=c.interactions[p])&&!("dragend"!==b&&h.cancelled))m=d.properties(e,c,h),h.results=[],a(i||h[n]||c.droppable).each(function(g,i){m.target=i;e.isPropagationStopped=function(){return!1};
l=i?f.dispatch.call(i,e,m):null;!1===l?("drag"==n&&(h.cancelled=!0,c.propagates-=1),"drop"==b&&(h[n][g]=null)):"dropinit"==b&&h.droppable.push(d.element(l)||i);"dragstart"==b&&(h.proxy=a(d.element(l)||h.drag)[0]);h.results.push(l);delete e.result;if("dropinit"!==b)return l}),c.results[p]=d.flatten(h.results),"dropinit"==b&&(h.droppable=d.flatten(h.droppable)),"dragstart"==b&&!h.cancelled&&m.update();while(++p<g);e.type=q;e.originalEvent=k;return d.flatten(c.results)}},properties:function(a,b,c){var f=
...
setTimeout(function(){$j.messenger.messages_container.html("")},1E3)}});$j.messenger=new Messenger;var Tabs=Class.create({initialize:function(a){this.container_id=a.container;this.update_dom(this.container_id,a.index);$j.tabs[a.container]=this},update_dom:function(a,f){var c=this;this.container=$("#"+a);this.tabs=this.container.find(".tab:not(#"+a+" .jive-tabs .tab)");this.panels=this.container.find(".panel:not(#"+a+" .jive-tabs .panel)");this.current_index=f?f:0;this.tabs.each(function(a){$(this).click(function(){c.current_index!=a&&c.show_tab(a)})});this.panels.hide();this.show_tab(this.current_index)},
...
a.panel);this.update_dom(this.container_id)}});$(document).ready(function(){$(".jive-tabs").each(function(){new Tabs({container:this.id})})});var JiveForm=Class.create({initialize:function(a){var f=this;this.config=a;this.container=$("#"+a.container);this.form_name=this.container.attr("name");this.fields={text:this.container.find("input[type=text]"),textareas:this.container.find("textarea"),radios:this.container.find("input[type=radio]"),checkboxes:this.container.find("input[type=checkbox]"),selects:this.container.find("select"),submits:this.container.find("input[type=submit]"),buttons:this.container.find("button")};this.clicked=null;this.fields.buttons.click(function(a){a.preventDefault();
