...
decimal:/^\d+\.\d{0,4}$/,price:/^\$?\s?[0-9\,]+(\.\d{0,4})?$/},init:function(){this.tabs={};this.forms={};this.msgs={};this.msg_index=0;this.slideshows={};this.crontab={};this.crontab_index=0;$(document).ready(function(){$j.init_dom()})},init_dom:function(){$(document).mousemove(this.get_mouse_position);this.get_viewport_dimensions();$(window).resize(this.get_viewport_dimensions)},get_mouse_position:function(a){$j.mouse={x:a.pageX,y:a.pageY}},get_viewport_dimensions:function(){$j.viewport={x:$(window).width(),
y:$(window).height()}},msg:function(a){this.messenger.create(a)},hello_world:function(){this.msg({type:"success",content:"<h2>Hello World!</h2><p>If you're seeing this message, it means jive is ready to use!</p>",sticky:1})},cron:function(a,b,c){c||(c=this.crontab_index,this.crontab_index++);var d={created:new Date,stop:function(){clearTimeout(this.timer)},timer:setInterval(function(){a(d)},b)};this.crontab[c]=d}};$j.init();window.$j.canopy={create:function(){$j.canopy.container=$('<div id="canopy"/>').append($('<div id="canopy-close-btn"/>').text("Close"));$j.canopy.overlay=$('<div id="canopy-overlay"/>');$("body").append($j.canopy.overlay);$("body").append($j.canopy.container);$j.canopy.container.find("#canopy-close-btn").click($j.canopy.close)},display:function(){$j.canopy.container||!$j.canopy.create();$j.canopy.overlay.show();$j.canopy.container.show()},close:function(){$j.canopy.overlay.hide();$j.canopy.container.hide()}};var Messenger=Class.create({initialize:function(){var a=this;this.container=$('<div id="jive-messenger"/>');this.messages_container=$('<div id="jive-messenger-messages"/>');this.close_btn=$('<div id="jive-messenger-close" style="display:none;"/>').text("x").click(this.close);$(document).ready(function(){a.container.append(a.messages_container);a.container.append(a.close_btn);$(document.body).append(a.container)})},create:function(a){var b=this,a={id:"msg-"+$j.msg_index,type:a.type?a.type:"notify",
...
"")})}}});$(document).ready(function(){$(".jive-form").each(function(){new JiveForm({container:this.id})})});