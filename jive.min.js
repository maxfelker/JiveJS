...
b})},textselect:function(b){a(document)[b?"unbind":"bind"]("selectstart",e.dontstart).css("MozUserSelect",b?"":"none");document.unselectable=b?"off":"on"},dontstart:function(){return!1},callback:function(){}};e.callback.prototype={update:function(){d.drop&&this.available.length&&a.each(this.available,function(a){d.drop.locate(this,a)})}};var m=b.dispatch;b.dispatch=function(b){if(0<a.data(this,"suppress."+b.type)-(new Date).getTime())a.removeData(this,"suppress."+b.type);else return m.apply(this,
...
new Date;$j.time={hours:a.getHours(),mins:a.getMinutes(),secs:a.getSeconds(),time_of_day:"am"};10>$j.time.mins?$j.time.mins="0"+$j.time.mins:$j.time.mins=$j.time.mins;10>$j.time.secs?$j.time.secs="0"+$j.time.secs:$j.time.secs=$j.time.secs;12<$j.time.hours&&($j.time.hours-=12,$j.time.time_of_day="pm")}};$j.init();window.$j.canopy={create:function(){$j.canopy.overlay=$('<div id="canopy"/>').append($('<div id="canopy-close"/>').text("x").click($j.canopy.close));$(document.body).append($j.canopy.overlay)},display:function(){0===$("#canopy").length&&$j.canopy.create();$j.canopy.overlay.show()},close:function(){$j.canopy.overlay.hide()}};var Messenger=Class.create({initialize:function(a){var b=this;this.container=$('<div id="jive-messenger"/>');this.messages_container=$('<div id="jive-messenger-messages"/>');this.close_btn=$('<div id="jive-messenger-close" style="display:none;"/>').text("x").click(this.close);$(document).ready(function(){b.container.append(b.messages_container);b.container.append(b.close_btn);$(document.body).append(b.container)})},create:function(a){var b=this;a={id:"msg-"+$j.msg_index,type:a.type?a.type:null,content:a.content,
timer:a.timer?a.timer:5E3,sticky:a.sticky?a.sticky:!1};a.container=$('<div class="jive-msg" id="'+a.id+'"/>').append($('<div class="jive-msg-contents"/>').html(a.content));a.type&&a.container.addClass("jive-msg-"+a.type);a.sticky?this.close_btn.show():this.close_btn.hide();this.messages_container.append(a.container);this.container.show();$j.msgs[$j.msg_index]=a;$j.msg_index++;a.sticky||setTimeout(function(){b.close()},a.timer)},clear:function(){this.close_btn.hide();this.messages_container.empty()},
close:function(a){$j.messenger.container.fadeOut();setTimeout(function(){$j.messenger.messages_container.html("")},1E3)}});$j.messenger=new Messenger;var Tabs=Class.create({initialize:function(a){var b=this;this.container_id=a.container;this.update_dom(this.container_id,a.index);this.check_hash();$j.tabbed[a.container]=this;window.onpopstate?window.onpopstate=function(a){b.check_hash()}:window.onhashchange=function(a){b.check_hash()}},check_hash:function(){var a=this,b=/#.+(?!\s)/g.exec(document.location);$.isArray(b)&&(tabID=b[0].replace("#",""),this.tabs.each(function(b){this.id=="tab-"+tabID&&a.show_tab(b)}))},update_dom:function(a,b){var d=
this;this.container=$("#"+a);this.tabs=this.container.find(".tab:not(#"+a+" .tabbed .tab)");this.panels=this.container.find(".panel:not(#"+a+" .tabbed .panel)");this.current_index=b?b:0;this.tabs.each(function(a){$(this).click(function(){d.current_index!=a&&d.show_tab(a)})});this.panels.hide();this.show_tab(this.current_index)},show_tab:function(a){var b=this;this.panels.hide().removeClass("current");this.tabs.removeClass("current");/^\d+$/.test(a)?this.current_index=a:this.tabs.each(function(d){this.id==
"tab-"+a&&(b.current_index=d)});var d="#"+this.tabs[this.current_index].id.replace("tab-","panel-");this.container.find("#"+this.tabs[this.current_index].id).addClass("current");this.container.find(d).show().addClass("current")},next_tab:function(){this.show_tab((this.current_index+1+this.tabs.length)%this.tabs.length)},previous_tab:function(){this.show_tab((this.current_index-1+this.tabs.length)%this.tabs.length)},add_tab:function(a){this.container.append(a.tab,a.panel);this.update_dom(this.container_id)}});
