...
resize:function(){this.width=$j.viewport.x/2;this.height=$j.viewport.y/2},close:function(){this.confirm_close?confirm("Are you sure you would like to close this window?")&&(this.container.remove(),delete $j.popups[this.container_id]):(this.container.remove(),delete $j.popups[this.container_id])},set_title:function(a){this.container.find(".popup-title").html(a)},set_contents:function(a){this.container.append(a)}});$j.modal={display:function(a){a=$.extend({container:"modal"},a);new Popup(a);this.popup=$j.popups.modal;$j.canopy.display();$("#canopy-close-btn").hide();$("#modal").find(".popup-close").addClass("btn btn-large");$("#modal").find(".popup-close").click(function(){$j.canopy.close()})},close:function(){$j.canopy.close();this.popup.close()}};