...
setTimeout(function(){$j.messenger.messages_container.html("")},1E3)}});$j.messenger=new Messenger;var Tabs=Class.create({initialize:function(a){var f=this;this.container=$("#"+a.container);this.tabs=this.container.find(".tab");this.panels=this.container.find(".panel");this.current_index=a.index?a.index:0;this.panels.hide();this.tabs.each(function(a){$(this).click(function(){f.current_index!=a&&f.show_tab(a)})});this.show_tab(this.current_index);$j.tabs[a.container]=this},show_tab:function(a){this.current_index=a;this.panels.hide();this.container.find("#"+this.tabs[a].id.replace("tab-","panel-")).show()},
next_tab:function(){this.show_tab((this.current_index+1+this.tabs.length)%this.tabs.length)},previous_tab:function(){this.show_tab((this.current_index+1+this.tabs.length)%this.tabs.length)}});$(document).ready(function(){$(".tabs-container").each(function(){new Tabs({container:this.id})})});var JiveForm=Class.create({initialize:function(a){var f=this;this.config=a;this.container=$("#"+a.container);this.form_name=this.container.attr("name");this.fields={text:this.container.find("input[type=text]"),textareas:this.container.find("textarea"),radios:this.container.find("input[type=radio]"),checkboxes:this.container.find("input[type=checkbox]"),selects:this.container.find("select"),submits:this.container.find("input[type=submit]"),buttons:this.container.find("button")};this.clicked=null;this.fields.buttons.click(function(a){a.preventDefault();
